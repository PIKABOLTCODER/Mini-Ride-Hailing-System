...

    {
        ...
        if (fp != NULL) {
            while (fgets(line, sizeof(line), fp) != NULL) {
                ...
                trimNewline(line);
                int result = sscanf(line, "%d|%d|%d|%d|%f|%f|%d|%d", &booking_id, &d_id, &p_id, &vehicle_type, &distance, &fare, &timestamp, &completed);
...
void addDriver(int id, char name[], int type, int x, int y) {
    Driver *d;
...
void addPassenger(int id, char name[], char mobile[]) {
    Passenger *p;
...
void displayTopDrivers(void) {
    ...
    arr = (Driver **)malloc((size_t)count * sizeof(Driver *));
    if (arr == NULL) {
    ...
    for (i = 0; i < count - 1; ++i) {
        int j;
        for (j = i + 1; j < count; ++j) {
            if (arr[j]->total_earnings > arr[i]->total_earnings) {
                Driver *tmp = arr[i];
                arr[i] = arr[j];
                arr[j] = tmp;
            }
...
void deleteDriver(int d_id) {
    ...
    while (leaf != NULL) {
        ...
        for (i = 0; i < leaf->keyCount; ++i) {
            Driver *d = (Driver *)leaf->records[i];
            if (d->d_ID != d_id) {
                newRoot = insert(newRoot, (long long)d->d_ID, d);
                if (newRoot == NULL) {
                    printf("Error: Failed to rebuild driver tree during deletion.\n");
                    return;
                }
            }
        }
...
void range_search(int p_id1, int p_id2) {
    ...
    if (low < 0 || high < 0) {
        printf("Passenger IDs must be non-negative.\n");
        return;
    }

    leaf = findLeaf(g_passengerRoot, (long long)low);
